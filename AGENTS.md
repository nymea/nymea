...
Do not run builds or test suites from this Codex environment; dependency setup is intentionally too complex here, so leave full project builds and validation to the user.
...
- Prefer `foreach (x, container)` over range-based `for (x : container)` to preserve C++11 compatibility across supported environments.
...
- In this Codex environment, do not run tests; document intended coverage and let the user execute validation locally.
