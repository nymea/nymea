    ...
    for (int i = 0; i < m_states.count(); ++i) {
        if (m_states.at(i).stateTypeId() == stateTypeId) {
            ...
            if (!newValue.convert(stateType.type())) {
                qCWarning(dcThing()).nospace() << this << ": Invalid value " << value << " for state " << stateType.name()
                                               << ". Type mismatch. Expected type: " << QVariant::typeToName(stateType.type()) << " (Discarding change)";
                return;
            }
            State state = m_states.at(i);
            if (state.minValue().isValid() && ThingUtils::variantLessThan(value, state.minValue())) {
                qCWarning(dcThing()).nospace() << this << ": Invalid value " << value << " for state " << stateType.name() << ". Out of range: " << state.minValue() << " - "
                                               << state.maxValue() << " (Correcting to closest value within range)";
            ...
            if (!stateType.possibleValues().isEmpty() && !stateType.possibleValues().contains(value)) {
                qCWarning(dcThing()).nospace() << this << ": Invalid value " << value << " for state " << stateType.name()
                                               << ". Not an accepted value. Possible values: " << stateType.possibleValues() << " (Discarding change)";
                return;
            }
            ...
            if (oldValue == newValue) {
                qCDebug(dcThing()).nospace() << this << ": Discarding state change for " << stateType.name() << " as the value did not actually change. Old value:" << oldValue
                                             << "New value:" << newValue;
                return;
            }
    ...
    for (int i = 0; i < m_states.count(); ++i) {
        if (m_states.at(i).stateTypeId() == stateTypeId) {
            ...
            if (ThingUtils::variantLessThan(m_states.at(i).maxValue(), newMin)) {
                qCWarning(dcThing()).nospace() << this << ": Adjusting state maximum value for " << stateType.name() << " from " << m_states.at(i).maxValue()
                                               << " to new minimum value of " << newMin;
                m_states[i].setMaxValue(newMin);
            }
            if (ThingUtils::variantLessThan(m_states.at(i).value(), newMin)) {
                qCInfo(dcThing()).nospace() << this << ": Adjusting state value for " << stateType.name() << " from " << m_states.at(i).value() << " to new minimum value of "
                                            << newMin;
        ...
        if (m_states.at(i).stateTypeId() == stateTypeId) {
            ...
            if (newMax.isValid()) {
                ...
                if (ThingUtils::variantGreaterThan(m_states.at(i).minValue(), newMax)) {
                    qCWarning(dcThing()).nospace() << this << ": Adjusting minimum state value for " << stateType.name() << " from " << m_states.at(i).minValue()
                                                   << " to new maximum value of " << newMax;
                    m_states[i].setMinValue(newMax);
                }

                if (ThingUtils::variantGreaterThan(m_states.at(i).value(), newMax)) {
                    qCInfo(dcThing()).nospace() << this << ": Adjusting state value for " << stateType.name() << " from " << m_states.at(i).value() << " to new maximum value of "
                                                << newMax;
        ...
        if (m_states.at(i).stateTypeId() == stateTypeId) {
            ...
            if (newMax.isValid() || newMax.isValid()) {
                ...
                if (ThingUtils::variantGreaterThan(newMin, newMax)) {
                    qCWarning(dcThing()).nospace() << this << ": Adjusting maximum state value for " << stateType.name() << " from " << m_states.at(i).maxValue()
                                                   << " to new minimum value of " << newMax;
                    m_states[i].setMaxValue(newMin);
                }
        ...
        if (m_states.at(i).stateTypeId() == stateTypeId) {
            ...
            if (!values.contains(m_states.value(i).value())) {
                if (values.contains(stateType.defaultValue())) {
                    qCInfo(dcThing()).nospace() << this << ": Adjusting state value for " << stateType.name() << " from " << m_states.at(i).value() << " to default value of "
                                                << stateType.defaultValue();
                    m_states[i].setValue(stateType.defaultValue());
                } else if (!values.isEmpty()) {
                    qCInfo(dcThing()).nospace() << this << ": Adjusting state value for " << stateType.name() << " from " << m_states.at(i).value() << " to new value of "
                                                << values.first();
