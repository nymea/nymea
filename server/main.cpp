...
int main(int argc, char *argv[])
{
    ...
    QCommandLineOption allOption(QStringList() << "p" << "print-all",
                                 QCoreApplication::translate("nymea", "Enables all info and debug categories except *Traffic and *Debug categories."));
    parser.addOption(allOption);

    QCommandLineOption logOption({"l", "log"},
                                 QCoreApplication::translate("nymea",
    ...
    QCommandLineOption dbusOption(QStringList() << "session",
                                  QCoreApplication::translate("nymea", "If specified, all D-Bus interfaces will be bound to the session bus instead of the system bus."));
    parser.addOption(dbusOption);

    QString debugDescription
        = QCoreApplication::translate("nymea",
                                      "Debug categories to enable. Prefix with \"No\" to disable. Suffix with \"Info\" or \"Warnings\" to address info and warning messages. "
                                      "Enabling a debug category will implicitly enable the according info category.\nExamples:\n-d ThingManager\n-d NoApplicationInfo\n\n");
    QCommandLineOption debugOption(QStringList() << "d" << "debug-category", debugDescription, "[No]DebugCategory[Warning|Info]]");
    parser.addOption(debugOption);

    QCommandLineOption interfacesOption({"i", "interface"},
                                        QCoreApplication::translate("nymea",
                                                                    "Additional interfaces to listen on. In nymea URI format (e.g. nymeas://127.0.0.2:7777). Note that such "
                                                                    "interfaces will not require any authentication as they are intended to be used for automated testing only."),
                                        "interfaceString");
    ...
    if (startForeground) {
        ...
        QObject::connect(NymeaCore::instance(), &NymeaCore::initialized, NymeaCore::instance(), []() { qCInfo(dcApplication()) << "The core is now up and running."; });
